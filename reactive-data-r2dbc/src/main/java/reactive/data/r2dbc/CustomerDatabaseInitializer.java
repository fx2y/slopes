...
public class CustomerDatabaseInitializer {
    ...
    CustomerDatabaseInitializer(@Value("classpath:/schema.sql") Resource resource, DatabaseClient client, SimpleCustomerRepository repository, TransactionalOperator operator) throws Exception {
        this.client = client;
    ...
    public Publisher<Void> resetCustomerTable() {
        ...
        Flux<Void> findAndDelete = repository.findAll().flatMap(customer -> repository.deleteById(customer.getId()));
