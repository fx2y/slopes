...
public class ConnectionManager {
    ...
    public <T> Flux<T> inConnection(Function<Connection, Flux<T>> action) {
        ...
        BiFunction<ConnectionCloseHolder, Throwable, Publisher<?>> closeBiFunction =
                ((connectionCloseHolder, throwable) -> closeFunction.apply(connectionCloseHolder));
        return Flux.usingWhen(connection(), holder -> action.apply(holder.connection), closeFunction, closeBiFunction, closeFunction);
    }
